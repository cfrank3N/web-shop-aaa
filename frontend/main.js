...
function loadCart() {
  ...
  if (localStorage.getItem("cart") === null) {
    ...
    for (let i in cart) {
          ...
          <div class="card p-3 border-0 rounded-5 shadow-sm">
            <div class="row g-0">
              <div class="col-md-2 position-relative">
                <img src="${
                  cart[i].image
                }" class="img-fluid position-absolute top-50 start-50 translate-middle rounded-4 cart-image-custom" alt="${
        cart[i].title
      }">
              </div>
              <div class="col-md-6">
                <div class="card-body ms-3">
                  ...
                  <span id="item-total" class="fw-bold">Item total: €${(
      ...
      if (i === 0) {
        //first row contains two columns, one for the product and one for order summary
        cartOutput += `
        <div class="col-sm-3">
...
const regexEmail = new RegExp(
  /^(?!.{50})[A-Za-zåäöÅÄÖ\.-_0-9]+@[A_Za-zåäöÅÄÖ\.-_0-9]+\.[a-z]{2,}$/
); //Done
...
function postOrder(e) {
  ...
  if (localStorage.getItem("cart") === null) {
    modalInner.innerHTML = `<div class="row d-flex align-items-center">
  ...
  fetch(request).then((response) => {
    if (!response.ok) {
      modalInner.innerHTML = `<div class="modal-body pt-4">
            <svg xmlns="http://www.w3.org/2000/svg" width="75" height="75" fill="#ff0000c3" class="bi bi-x-circle-fill" viewBox="0 0 16 16">
              <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M5.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293z"/>
            </svg>
          </div>
      ...
      <div class="card h-100 shadow scale-on-hover rounded-5" role="button">
        <div class="card-body" data-bs-toggle="modal" data-bs-target="#productModal" onclick="populateProductPopUp(${i})">
          ...
          <div class="position-relative mt-3 card-image-container">
            <img src="${
              products[i].image
            }" class="card-img-top card-image-custom position-absolute top-50 start-50 translate-middle img-fluid w-75 object-fit-contain" alt="${
        products[i].title
      }">
          </div>
          <!-- info -->
...
function populateProductPopUp(index) {
  ...
  document.getElementById("modal-price").textContent = `€${products[
    index
  ].price.toFixed(2)}`;
  ...
  document.getElementById(
    "buy-button"
  ).innerHTML = `<button class="btn btn-custom px-4 py-2 rounded-5" onclick="addToCart(${index})">Add to cart</button>`;
}
...
async function toggleAdminPanel() {
  ...
  try {
    ...
    if (response.ok && adminListItem === null) {
      ...
      const adminText = document.createElement("a");
